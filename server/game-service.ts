...
type Player = PublicPlayer & { token: string; disconnectedAt?: number };
type Room = {
...
export class GameService {
  ...
  expireDisconnect(code: string, playerId: string, disconnectedAt: number) {
    ...
    if (player.isHost && room.phase !== 'lobby' && room.phase !== 'finished' && room.phase !== 'interrupted') {
  ...
  start(code: string, playerId: string) {
    ...
    room.players.sort((a, b) => a.timeline[0].song.year - b.timeline[0].song.year);
  ...
  private newPlayer(nickname: string, isHost: boolean): Player {
    const clean = nickname.trim().slice(0, 24);
    if (clean.length < 2) throw new Error('Choose a nickname with at least two characters.');
    return { id: randomUUID(), token: randomBytes(24).toString('base64url'), nickname: clean, isHost, connected: true, score: 0, tokens: 2, timeline: [] };
  }
