...
export default function App() {
  ...
  const action = async (event: string, ...args: unknown[]): Promise<Ack> => {
    ...
    try { const ack = await emit(event, ...args); if (!ack.ok) setNotice(ack.error ?? t('actionFailed')); return ack; } finally { actionPending.current = false; }
  };
  ...
  return <Game state={state} playerId={session.playerId} isHost={isHost} notice={notice || spotify.error} spotify={spotify} startRound={startRound} retryPlayback={retryPlayback} replaceSong={replaceSong} action={action} connectSpotify={connectSpotify} disconnectSpotify={() => { disconnectSpotify(); location.reload(); }} leave={leave} shareRoom={shareRoom} locale={locale} setLocale={setLocale} t={t} />;
}
